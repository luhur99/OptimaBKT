...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    IF OLD.status IN ('delivered'::public.delivery_order_status, 'cancelled'::public.delivery_order_status) THEN
    ...
    IF OLD.status = 'pending'::public.delivery_order_status THEN
      IF NEW.status NOT IN ('on_delivery'::public.delivery_order_status, 'delivered'::public.delivery_order_status, 'cancelled'::public.delivery_order_status) THEN
        RAISE EXCEPTION 'Invalid status transition from PENDING to %.', NEW.status;
      END IF;
    -- Allow transitions from on_delivery
    ELSIF OLD.status = 'on_delivery'::public.delivery_order_status THEN
      IF NEW.status NOT IN ('delivered'::public.delivery_order_status, 'cancelled'::public.delivery_order_status) THEN
